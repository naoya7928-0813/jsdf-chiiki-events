...
**採った対応: 告知（更新ノート＋開いたときのお知らせ）**

- `src/constants/updates.js` に「お気に入り・設定・通知は引き継がれない」旨を記載
- `src/components/DomainNotice.jsx` が**開いたときに一度だけ**知らせる。
  出す条件と文面は `shared/domainNotice.cjs` が決める:

  | 見ているドメイン | 文面 |
  |---|---|
  | 旧（`LEGACY_ORIGINS`）| 「引っ越しました」＋新しいサイトへ移動するボタン |
  | 新（`DEFAULT_SITE_URL`）| 「アドレスが変わりました。設定は引き継がれません」|
  | localhost・プレビュー・`www` | 出さない |

  閉じた記録は localStorage（`jsdf-domain-notice`）。`SHOW_UNTIL`（2026-12-31）を
  過ぎたら出さない（移行から時間が経つと新規利用者には無関係な話になるため）。

⚠ **旧ドメインのお知らせは、308 リダイレクトを入れると出なくなる**（旧ドメインで
アプリが動かなくなるため）。リダイレクトは、このお知らせが行き渡ってから設定すること。

利用者が増えたら、旧ドメインの「新しいサイトへ移動」ボタンに localStorage の設定を
URL フラグメントで載せて引き継ぐ導線も検討できる（実装は半日程度）。
その場合も **リダイレクトを入れる前に**用意すること。
...
### 別ドメイン（`.com` など）を取ってリダイレクトする場合

打ち間違い・別TLDからの流入を拾うために `jsdf-chiiki-events.com` のような
ドメインを取って `.jp` へ寄せることができる。**コード側の変更は不要**
（リダイレクトは Vercel のエッジで完結し、アプリはそのドメインで動かない）。

1. そのドメインを取得する
2. **ネームサーバーを `.jp` と同じ Vercel のものに向ける**
   （エックスサーバーの場合: Xserver アカウント → ドメイン → ネームサーバー設定）
3. Vercel → Settings → Domains → **Add Domain** で追加
   - ⚠ **「Redirect apex domains to www」のチェックは外す**
   - **Redirect to Another Domain** → `jsdf-chiiki-events.jp` / **308 Permanent** を選ぶ
4. `www.<新ドメイン>` も受けるなら同様に追加して同じ先へ 308

- `SITE_ORIGINS` に足す必要は**ない**。リダイレクトされるだけで、そのオリジンから
  書き込みAPIを呼ぶことはないため（足すと不要な許可オリジンを増やすことになる）
- canonical は `.jp` のままでよい。リダイレクト先が正規URLであることは 308 で伝わる
- `LEGACY_ORIGINS` にも足さない。移行元ではないので、移行のお知らせを出す対象ではない
