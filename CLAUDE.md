...
### 記載タイミング（自動運用）
- **修正・実装を行ったら、その都度ユーザーに確認を取らずに自動で更新ノートへ追記し、`package.json` のバージョンも上げる。**
- ユーザー影響のある変更（機能追加・対応範囲拡大・UI改善・バグ修正）はすべて対象。
- 純粋に内部的でユーザーに影響しないもの（コメント修正・内部リファクタ・デバッグスクリプト等）は記載不要。
