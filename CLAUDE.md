...
### 記載タイミング
- **更新ノートに記載するのは「ユーザーにとって意味のある大きな変更」のときのみ。**
  - 例: 「埼玉県の募集案内所のイベント情報を掲示し始めました」のような、新しい情報提供・新機能・対応範囲の拡大など、利用者が体感できる変化。
- 次のような変更は原則として**記載しない**（バージョンも上げない）:
  - バグ修正・不具合対応（例: スクレイプの取りこぼし修正、0件上書き防止）
  - 表記調整・軽微なUI変更・誤字修正・内部リファクタリング
  - 掲載元一覧の整備など、既存機能の内部的な拡充
