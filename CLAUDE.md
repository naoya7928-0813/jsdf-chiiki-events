...
### 記載タイミング（自動運用・ユーザビリティに影響する変更のみ）
- **ユーザビリティ（利用者が体感できる挙動・表示）に影響する変更を行ったら、その都度ユーザーに確認を取らずに自動で更新ノートへ追記し、`package.json` のバージョンも上げる。**
- 対象（記載する）: 機能追加・対応範囲の拡大・画面/表示の変更・UI改善・利用者に見えるバグ修正（例: イベントが表示されない不具合の修正）。
- **対象外（記載しない・バージョンも上げない）= 裏側（バックエンド）の変更**:
  - スクレイパー内部の改修、OCR用ツールの追加・差し替え、パーサーのリファクタなど、**表示結果が変わらない内部的な変更**。
  - 例: 「OCR用のツールを増やした」「Cloudflare回避ロジックを調整した」などは裏側のためノートに載せない。
  - コメント修正・デバッグスクリプト・CI設定など。
- 判断基準: その変更で**利用者の画面・表示・取得できる情報が変わるか**。変わるなら記載、変わらない（裏側のみ）なら記載しない。
