...
- スケジュール: 日本時間 **05:33 / 09:33 / 15:33 開始**（1日3回。scrape.yml の cron `33 20 / 33 0 / 33 6` = UTC）
  ...
  - **通知は 08:00〜09:00 / 12:00〜13:00 / 18:00〜19:00(JST) の「配信枠」で送る**（2026-09-05 に点→窓へ変更）。
    運用の考え方は「**枠の中で掲載する**」であって開始時刻ちょうどを狙うのではない。
    枠の開始より早くは配信せず（ジョブ内で `sleep`）、枠の終わりまでに掲載する。
    枠の長さは `WINDOW_MINUTES`（60分）
  - 根拠（2026-08-30 に Actions API で実測）: ジョブ全体 **61.6 / 62.3 / 63.4 分**（準備40秒＋本体約60分＋
    ...
    最小17分・中央値66分・p75 122分。
    枠の開始の147分前に起動し、枠が60分あるので **143分の遅延まで枠内**に収まる（実測 p75 の122分をカバー）。
    点だった頃は83分までで、実測30回中20回＝67%しか収まらず 09:32 のような時刻に出ていた
...
| 決めごと | 値・場所 | 理由 |
|---|---|---|
| 打ち切り時刻 | **配信枠の終わり**の `RESERVE_MINUTES`（6分）前 | 打ち切り後に品質チェック→commit→デプロイ→CDN伝播(60秒)が要る（実測約3分）。倍の余裕。**枠の開始ではなく終わりから逆算する**（点→窓の変更で締切が60分うしろへ延び、見送る地本が減った） |
| 締切を使わない条件 | 使える時間が `MIN_USEFUL_MINUTES`（15分）未満 | 起動が遅すぎる回で打ち切ると「何も更新せず新着ゼロ」を枠内配信するだけになる。**最後まで走らせて遅れて配信**する |
| 「定刻外」の判定 | **枠の終わりを過ぎたとき**だけ | 開始を過ぎただけで警告を出すと、想定どおりの配信まで定刻外になる |
| 手動実行 | 締切なし | 狙うスロットが無い |
...
⚠️ **変換表に日本語と同じ字（自己写像）を入れないこと**。何も起きないうえ、
「その字は対策済み」と誤解させる（テストが自己写像を検出して落とす）。
⚠️ 対応先が割れる字（`历`＝歴/暦、`发`＝発/髪）も入れない。誤変換になる。

**表記ゆれ（`normalizeJaText`）**: 半角カナ（`｢｣ﾊﾞ` 等）を全角へ、日本語以外の
中点（`·` U+00B7・`•`）を `・` へ揃える。`cleanEventTitle` / `cleanPlaceText` が通す。
⚠️ **文字列全体に NFKC をかけないこと**。全角の記号・英数字まで巻き込み
