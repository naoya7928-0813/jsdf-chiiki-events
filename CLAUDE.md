...
### 記載タイミング（基準＝利用者に影響があるか）
- **利用者に影響がある更新は内容を記載する**（＋ `package.json` のバージョンを上げる）。ユーザー確認は取らず自動で行う。
  - 例: 機能追加・対応範囲の拡大・画面/表示の変更・利用者に見えるバグ修正など。
- **利用者に影響がない更新は記載しない**（バージョンも上げない）。
  - 例: 裏側（バックエンド）の変更＝スクレイパー内部改修・OCRツールの追加/差し替え・パーサーのリファクタ・Cloudflare回避調整・CI設定・デバッグスクリプト等、表示結果が変わらないもの。軽微な内部修正も同様。
- 記載する内容は簡潔に書く。
- 判断基準はシンプルに「**その更新が利用者に影響するか（画面・表示・取得情報・操作感が変わるか）**」。変わる→記載、変わらない→記載しない。
