...
const SIMPLIFIED_TO_JP = {
  ...
  袜: '襪', 赛: '賽', 郑: '鄭', 酱: '醤', 释: '釈', 雾: '霧',
  // 2026-09-05 追加。公開データに「专用八無料送迎。」（专＝専）が出ていたため、
  // 同種の取りこぼしをまとめて塞ぐ。日本語に同じ字が無いものだけを選んでいる
  // （历＝歴/暦、发＝発/髪 のように対応先が割れる字は入れない。誤変換になるため）。
  专: '専', 业: '業', 产: '産', 众: '衆', 丰: '豊', 临: '臨', 乡: '郷', 义: '義',
  亚: '亜', 从: '従', 仓: '倉', 仪: '儀', 伟: '偉', 伤: '傷', 优: '優', 华: '華',
  协: '協', 卫: '衛', 厅: '庁', 压: '圧', 园: '園', 圆: '円', 坚: '堅', 奖: '奨',
  妇: '婦', 宁: '寧', 宪: '憲', 审: '審', 层: '層', 岛: '島', 币: '幣', 师: '師',
  广: '広', 库: '庫', 张: '張', 归: '帰', 总: '総', 恶: '悪', 户: '戸', 扬: '揚',
  报: '報', 拟: '擬', 择: '択', 挥: '揮', 换: '換', 摄: '摂', 杂: '雑', 步: '歩',
  毁: '毀', 气: '気', 泽: '沢', 渐: '漸', 滨: '浜', 炮: '砲', 烧: '焼', 牵: '牽',
  狮: '獅', 猎: '猟', 监: '監', 矿: '鉱', 窗: '窓', 粮: '糧', 罗: '羅', 职: '職',
  聪: '聡', 胜: '勝', 荐: '薦', 讯: '訊', 赏: '賞', 辉: '輝', 邮: '郵', 韩: '韓',
  页: '頁',
};

/**
 * 日本語の表記ゆれをならす（意味は変えない・文字種だけ揃える）。
 *
 * 地本サイトとチラシOCRから来る文字は出どころがばらばらで、同じ内容でも
 * 見た目が揃わない。カード上で不揃いに見えるものだけを対象にする。
 *
 *   半角カナ  … 「｢はたらく車大集合!｣」→「「はたらく車大集合!」」
 *               濁点付き（ﾊﾞ→バ）も正しく合成される
 *   中点      … 「中高校生·大学」→「中高校生・大学」
 *               `·`(U+00B7) は欧文・中国語の区切りで、日本語の `・` とは別字
 *
 * ⚠ 文字列全体に NFKC をかけないこと。全角の記号や英数字まで巻き込んで
 *   「（）」→「()」のように出典の表記を壊す。半角カナの連なりだけに限定する。
 */
function normalizeJaText(raw) {
  const s = String(raw ?? '');
  if (!s) return s;
  return s
    .replace(/[｡-ﾟ]+/g, m => m.normalize('NFKC'))   // 半角カナ・半角括弧
    .replace(/[·•∙]/g, '・');                        // 日本語以外の中点
}
...
function cleanEventTitle(raw) {
  ...
  // あわせて半角カナ・中点の表記ゆれもならす。
  let t = normalizeJaText(toJapaneseKanji(String(raw))).replace(/\s+/g, ' ').trim();
...
function isSuspiciousTitle(title) {
  ...
  // 句点で終わる＝イベント名ではなく本文の一文（「専用八無料送迎。」等）。
  // 実データ224件のうち句点で終わるタイトルはこの1件だけで、いずれも本文の断片だった
...
function cleanPlaceText(raw) {
  ...
  // 会場名も簡体字を日本語へ直し、半角カナ・中点の表記ゆれをならす
  // （「关山演习場」→「関山演習場」、「中高校生·大学」→「中高校生・大学」）
  let p = normalizeJaText(toJapaneseKanji(String(raw))).replace(/\s+/g, ' ').trim();
