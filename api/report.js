...
//
// 設計（2026-07-17 ntfy から運営コンソールへ移行）:
// - 報告本体（個人情報を含みうる：連絡先メール・端末情報）は **公開型の ntfy には載せず**、
//   サーバー側の Redis に保存する。運営者は認証付き管理画面（GET/PATCH/DELETE）で閲覧する。
// - ntfy へは「新着が1件届いた」事実（種別のみ・本文/連絡先/端末情報なし）だけを任意で通知する。
//   NTFY_BUG_TOPIC 未設定なら通知は行わない（保存は継続）。
