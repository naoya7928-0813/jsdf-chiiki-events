...
/**
 * 和歌山地本 WordPress 投稿ページのパーサー（1投稿 = 1イベント）
 *
 ...
 */
function parseWakayamaPost($, url, counter) {
  ...
  if (!dateStr) {
    ...
    if (!flyerUrl) return [];
    return [{
      id: `wk-flyer-${counter}`, pref: 'wakayama', date: '', weekday: '',
  ...
  // ── 場所 ─────────────────────────────────────────────────────
  const placeM = halfBody.match(/(?:場所|会場|開催場所)[：: ]\s*(.{2,60?})(?:\s+(?:日時|内容|[●■])|$)/);
  ...
  return [{
    id:             `wk-${dateStr.replace(/-/g, '')}-${counter}`,
    pref:           'wakayama',
    date:           dateStr,
    weekday,
...
/**
 * 和歌山地本イベント一覧ページから投稿 URL を抽出する。
 */
function parseWakayamaPostUrls($) {
  const urls = [];
  $('a[href*="mod.go.jp/pco/wakayama/post-"]').each((_, a) => {
...
module.exports = { parseWakayamaPost, parseWakayamaPostUrls };
